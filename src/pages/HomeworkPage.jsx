...
const ROUTES = {
  LIST: '/homework',
  HUB: '/homework/:id',
  PREVIEW: '/homework/:id/preview',
  RUN: '/homework/:id/run',
  RESULTS: '/homework/:id/results',
  REPORT: '/homework/:id/report',
...
export default function HomeworkPage({ activeSubMenu, onBackToSummary, onViewerModeChange }) {
  const [route, setRoute] = useState(ROUTES.LIST);
  const [activeId, setActiveId] = useState('h-201');
  ...
  return (
    <div
      className={`${isViewerRoute ? 'p-0 bg-gray-100 h-screen overflow-hidden' : 'p-6 bg-emerald-50 min-h-screen'}`}
    ...
    >
      {route === ROUTES.LIST && (
        <AssessmentListPage
          ...
          assessmentKind={AssessmentKind.HOMEWORK}
          pageLabel="숙제"
        />
      )}
      {route === ROUTES.HUB && (
